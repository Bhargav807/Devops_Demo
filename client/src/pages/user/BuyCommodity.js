...
const BuyCommodity = () => {
    ...
    const handlepropose = async (pid, sellerid) => {
        try {
            ...
            const { data } = await axios.post(`${process.env.REACT_APP_API}/api/v1/products/propose`, { buyerid, pid, sellerid });
            if (data?.success) {
                            ...
                            <div className="col-lg-3 col-md-3 col-sm-6 m-4" key={p._id}>
                                ...
                                <div className="card" style={{ width: "18rem" }}>
                                    ...
                                    <div className="card-body">
                                        <h5 className="card-title" style={{fontSize:"0.8rem"}}>{p.name}<br />product id: {p._id}<br />seller id : {p.sellerId._id}<br/>{p.sellerId.phone}<br/>{p.description}</h5>
                                        <p className="card-text"><span className='text-dark bg-warning'>{p.quality} Rs.{p.price}/-</span></p>
                                        <button className={`btn m-2 btn-${proposedlist.includes(p._id) ? "danger" : "primary"} btn-sm`} onClick={() => {
                                            if (proposedlist.includes(p._id)) {
                                                ...
                                                handlepropose(p._id, p.sellerId._id); // Otherwise, call handlePropose
                                            }
                                        }}>
